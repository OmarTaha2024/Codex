        ...
        private void LoadProjectswithStudents()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string query = @"
                    SELECT concat (Courses.Name,'- Round ', Courses.RoundNumber) As course, Students.Name AS Student, Sessions.SessionName,Projects.ProjectName, Projects.Grade
                    FROM     Courses INNER JOIN
                  Sessions ON Courses.CourseID = Sessions.CourseID AND Courses.RoundNumber = Sessions.RoundNumber INNER JOIN
                  Projects ON Sessions.SessionID = Projects.SessionID CROSS JOIN
                  Students
        ...
        private void LoadProjects()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string query = @"
                    SELECT concat ( Courses.Name,' - Round ', Courses.RoundNumber) AS course , Sessions.SessionName, Projects.ProjectName
                    FROM     Courses INNER JOIN
                  Sessions ON Courses.CourseID = Sessions.CourseID AND Courses.RoundNumber = Sessions.RoundNumber INNER JOIN
                  Projects ON Sessions.SessionID = Projects.SessionID
