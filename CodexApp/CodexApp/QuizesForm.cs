        ...
        private void LoadQuizesswithStudents()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string query = @"
                      SELECT concat (Courses.Name,'- Round ', Courses.RoundNumber) As course, Students.Name AS Student, Sessions.SessionName,Quizzes.QuizName, Quizzes.Grade
                    FROM     Courses INNER JOIN
                    Sessions ON Courses.CourseID = Sessions.CourseID AND Courses.RoundNumber = Sessions.RoundNumber INNER JOIN
                    Quizzes ON Sessions.SessionID = Quizzes.SessionID CROSS JOIN
                    Students
        ...
        private void LoadQuizes()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    string query = @"
                     SELECT concat ( Courses.Name,' - Round ', Courses.RoundNumber) AS course , Sessions.SessionName, Quizzes.QuizName
                     FROM     Courses INNER JOIN
                    Sessions ON Courses.CourseID = Sessions.CourseID AND Courses.RoundNumber = Sessions.RoundNumber INNER JOIN
                    Quizzes ON Sessions.SessionID = Quizzes.SessionID
